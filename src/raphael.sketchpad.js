 ...
 *
 * Versions:
 * 0.5.2.HYH - HYH Customisations SDUFFY
...
(function(Raphael) {
	...
	var SketchPad = function(paper, options) {
		...
		self.json_viewer = function(value) {
			if (value === undefined) {
				for (var i = 0, n = _strokes.length; i < n; i++) {
					var strokes_local = _strokes;
					var stroke = strokes_local[i];
					// SDUFFY Fill the shape
					strokes_local[i].fill = strokes_local[i].stroke;
					if (typeof stroke.path == "object") {
						stroke.path = svg_path_to_string(stroke.path);
					}
				}
				console.log(strokes_local);
				console.log(strokes_local[0].fill);
				return JSON.stringify(strokes_local);
			}
